...
func _ready():
	add_to_group("food")
	update_visual()
	print("[FOOD] Spawned: %d units at (%.0f, %.0f)%s" % [
		food_amount, 
		global_position.x, 
		global_position.y,
		" [INFINITE]" if is_infinite else ""
	])
...
func take_food(amount: int = 1) -> bool:
	if is_infinite:
		total_food_taken += amount
		food_taken.emit(amount, 999)
		print("[FOOD] Took %d from INFINITE source (total taken: %d)" % [amount, total_food_taken])
		return true
	...
	print("[FOOD] Took %d, %d remaining at (%.0f, %.0f)" % [
		amount, 
		food_amount, 
		global_position.x, 
		global_position.y
...
func deplete():
	...
	print("[FOOD] ✗ DEPLETED at (%.0f, %.0f) - Total taken: %d" % [
		global_position.x,
		global_position.y,
		total_food_taken
	])
...
func refill():
	...
	print("[FOOD] ✓ Refilled to %d at (%.0f, %.0f)" % [
		food_amount,
		global_position.x,
		global_position.y
	])
